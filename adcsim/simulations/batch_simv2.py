...
def simv2_loop(mag_size, output_file, time_step):
    ...
    # time = np.arange(end_time//2, end_time, time_step)
    ...
    cubesat = CubeSatModel(inertia=np.diag([0.1, 0.06, 0.003]), magnetic_moment=np.array([0, 0, mag_size]),
                           hyst_rods=[rod1, rod2])
    ...
    ab = np.concatenate((orbit_data.sun.values, orbit_data.mag.values, orbit_data.atmos.values.reshape(a, 1),
                         ...
                         orbit_data.alts.values.reshape(a, 1), orbit_data.positions.values,
    ...
    for i in tqdm(range(len(time) - 1)):
        if not i % save_every:
            ...
            sun_vec[k], mag_field[k], density[k], lons[k], lats[k], alts[k], positions[k], velocities[k] = interp_info(
                i)
            ...
            states[k + 1] = it.rk4(st.state_dot_mrp, time_step, state, controls[k], cubesat.inertia,
                                   cubesat.inertia_inv)
    ...
    a.to_netcdf(os.path.join(os.path.dirname(__file__), output_file))
...
for i in range(11, 21):
    ...
    if mag_strength > 10:
        ...
        ts = 0.01
        filename = '..\\..\\' + '\\time_step_check\\' + str(i/2) + '_G_' + str(ts) + '_ts.nc'
        simv2_loop(mag_strength, filename, ts)
